...
const AppointmentItem = ({
  ...
  return (
    <div
      className={`mb-2 cursor-pointer rounded-md border p-2 transition-shadow hover:shadow-md ${getTypeColor()}`}
...
const DayView = ({
  ...
  return (
    <div className="mb-6">
      <div className={`mb-2 border-b pb-1 text-lg font-medium ${isToday ? 'text-blue-600' : ''}`}>
        {date.toLocaleDateString('pt-BR', { weekday: 'long', day: '2-digit', month: 'long' })}
        {isToday && (
          <span className="ml-2 rounded-full bg-blue-100 px-2 py-0.5 text-sm text-blue-800">
            Hoje
          </span>
        )}
      </div>
      ...
      {dayAppointments.length > 0 ? (
        ...
        <div className="py-4 text-center text-gray-500">Nenhum agendamento para este dia</div>
      )}

      {/* Linha do tempo (apenas para desktop) */}
      <div className="relative ml-4 mt-4 hidden border-l border-gray-200 pl-4 md:block">
        {hours.map((hour) => {
          ...
          const hourAppointments = dayAppointments.filter(
            (appointment) => appointment.start.getHours() === hour
          )

          return (
            <div key={hour} className="mb-4">
              <div
                className={`-ml-6 flex items-center ${isCurrentHour ? 'font-medium text-blue-600' : ''}`}
              >
                <div
                  className={`h-4 w-4 rounded-full ${isCurrentHour ? 'bg-blue-600' : 'bg-gray-300'}`}
                ></div>
              ...
              {hourAppointments.length > 0 && (
                <div className="ml-2 mt-1">
                  {hourAppointments.map((appointment) => (
                    <div
                      key={appointment.id}
                      className="mb-1 cursor-pointer rounded bg-gray-100 p-1 text-xs hover:bg-gray-200"
  ...
  return (
    <div className="space-y-4 border-4 border-red-500">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        ...
        <div>
          <label className="mb-1 block text-sm font-medium text-gray-700">Clínica</label>
          <Select
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        ...
        <div>
          <label className="mb-1 block text-sm font-medium text-gray-700">Hora Início</label>
          <input
            type="time"
            value={
              formData.start
        ...
        <div>
          <label className="mb-1 block text-sm font-medium text-gray-700">Hora Fim</label>
          <input
            type="time"
            value={
              formData.end
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div>
          ...
          <Select
            value={formData.type}
            onValueChange={(value: AppointmentEvent['type']) => handleChange('type', value)}
          >
            <SelectTrigger>
        ...
        <div>
          ...
          <Select
            ...
            onValueChange={(value: AppointmentEvent['status']) => handleChange('status', value)}
          >
            <SelectTrigger>
      ...
      <div className="flex justify-between pt-2">
        ...
        <div className="flex space-x-2">
          ...
          <Button onClick={() => onSave(formData)}>
            {isNew ? 'Agendar' : 'Salvar Alterações'}
          </Button>
        </div>
      </div>
    </div>
...
export default function AgendaPage() {
  ...
  const [selectedAppointment, setSelectedAppointment] = useState<AppointmentEvent | undefined>(
    undefined
  )
  ...
  return (
    <div className="border-2sm:space-y-6 space-y-4">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        ...
        <Button onClick={openNewAppointmentForm} className="w-full sm:w-auto">
            ...
            <div className="flex items-center space-x-2">
              ...
              <div className="font-medium">
                ...
                {viewMode === 'week' && (
                  <span>
                    Semana de{' '}
                    {getDaysToShow()[0].toLocaleDateString('pt-BR', {
                      day: '2-digit',
                      month: '2-digit',
                    })}{' '}
                    a{' '}
                    {getDaysToShow()[6].toLocaleDateString('pt-BR', {
                      day: '2-digit',
                      month: '2-digit',
                    })}
                  </span>
            ...
            <div className="space-y-6">
              {getDaysToShow().map((day, index) => (
                <DayView
                  ...
                  onAppointmentClick={openEditForm}
                />
              ))}
            </div>
          </div>
